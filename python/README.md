...
`build_room_state()` omits unknown sensor values, including JSON `null`, just like the TypeScript core. It rejects malformed shapes and out-of-vocabulary movement, posture, boolean, and robot gaze labels before they can enter model state. Recent transcript text is capped at 200 UTF-16 units without splitting a Unicode scalar; lone surrogates in the retained prefix are rejected. This is a size boundary, not sanitization or consent. It does not infer missing observations or establish sensor accuracy.
